...
void Aerial_Device::get_joint_pose(int i, Matrix* pose)
{
	...
	//autobots[0]-transform(pose, true);
}
...
Matrix Aerial_Device::update_odometry(float t)
{
	...
	for(int i=0; i < accel.size() / 2; i++)
	{
		...
		stateActual.set(0, i, stateActual.get(0,i) + (stateActual.get(1, i) * t) + (accel[i] * pow(t, 2.0) / 2.0));
		stateActual.set(1, i, stateActual.get(1, i) + (accel[i] * t));
		accel[i+3] = accel[i+3] / mass;
		stateActual.set(0, i+3, wrap_angle(stateActual.get(0,i+3) + (stateActual.get(1, i+3) * t) + (accel[i+3] * pow(t, 2.0) / 2.0)));
