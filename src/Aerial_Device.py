...
class Controller():
	...
	def tune(self, device, horizon=1000):
		...
		for i in range(horizon):
			adjust_omegas(target, device.poseActual, device.velocity, device.net_force)
...
class Device():
	...
	def render_device(self, ax, colors=['r', 'b'], verbose=0):
		...
		# print status
		if verbose > 0:
			print(f'New Pose:\n\tX : {self.poseActual[0]} \tY: {self.poseActual[1]} \tZ: {self.poseActual[2]} \tPhi: {self.poseActual[3]} \tTheta: {self.poseActual[4]} \tPsi: {self.poseActual[5]}')
			print(f'Net Forces:\n\tX : {self.net_force[0]} \tY: {self.net_force[1]} \tZ: {self.net_force[2]} \tPhi: {self.net_force[3]} \tTheta: {self.net_force[4]} \tPsi: {self.net_force[5]}')
	...
	def render_velocity(self,ax,colors):
		self.populate_ax_from_id(ax, self.id.path['velocities'], colors, label='Velocity', title='Velocity over time')

	def render_acceleration(self, ax, colors):
		self.populate_ax_from_id(ax, self.id.path['accelerations'], colors, label='Acceleration', title='Acceleration over time')
