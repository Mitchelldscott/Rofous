...
impl Sockage {
    ...
    pub fn shutdown_packet(&self) -> [u8; 2] {
        [13, self.client_id]
    }
}
...
impl SockServer {
    pub fn core() {
        ...
        while !shutdown {
                        ...
                        Ok(size) => {
                            match size {
                                SEND_PACKET_SIZE => {
                                    ...
                                    match socks
                                        .iter()
                                        .find(|sock| sock.client_id == new_sock.client_id)
                                    {
                                        ...
                                        None => {
                                            if buffer[0] == 255 {
                                                ...
                                                new_sock.millis_rate =
                                                    ((1.0 / rate) * 1000.0) as u32;

                                                if buffer[2] != 64 {
                                                    match new_sock.targets.read().unwrap().len() {
                                                        0 => {
                                                            *new_sock.targets.write().unwrap() = (0
                                                                ..buffer[2])
                                                                .map(|i| buffer[(i + 3) as usize])
                                                                .collect();
            ...
            (0..socks.len()).for_each(|i| match socks[i].is_shutdown() {
                true => dead_socks.push(i),
                false => {
                    shutdown = false;
                        ...
                        socks[i]
                            ...
                            .for_each(|(j, target)| {
                                if (*target as usize) < socks.len() {
                                    ...
                                    socks[i].send_buffer.write().unwrap().puts(
                                        3 + (64 * j),
                                        socks[*target as usize]
                                            .receive_buffer
                                            .read()
                                            .unwrap()
                                            .buffer()[3..67]
            ...
            match stream.read(&mut buffer) {
                Ok(size) => match size {
                    SEND_PACKET_SIZE => {
                        t = Instant::now();
                        match buffer[0] {
                            255 => sock.receive(buffer[0..64].to_vec()),
                            13 => {
                                println!(
                                    "[Sock-thread] {}: Received Kill code from {}",
                                    addr, buffer[1]
                                );
                                sock.shutdown();
                                break;
...
impl SockClient {
    pub fn sock_handle(sock: Sockage) {
        ...
        sock.send_buffer.write().unwrap().puts(
            0,
            [
                255,
                sock.client_id,
                sock.targets.read().unwrap().len() as u8,
            ]
            .into_iter()
            .chain(sock.targets.read().unwrap().iter().map(|t| *t))
            .collect(),
        );
        sock.send_buffer
            .write()
            .unwrap()
            .put_float(1020, (1.0 / sock.millis_rate as f64) * 1000.0);

        stream
            .write_all(sock.send_buffer.read().unwrap().buffer())
            .unwrap();
        stream.flush().unwrap();
        ...
        SockClient::terminate(&mut stream, sock);
    }
    ...
    pub fn request_publish(client_id: u8, rate: f64, data: Vec<u8>) {
        ...
        sock.send_buffer.write().unwrap().puts(
            0,
            [255, sock.client_id, 64]
                .into_iter()
                .chain(data.into_iter().take(64))
                .collect(),
        );
    ...
    pub fn terminate(stream: &mut TcpStream, sock: Sockage) {
        sock.shutdown();
        stream.write_all(&sock.shutdown_packet()).unwrap();
        stream.flush().unwrap();
    }
}
